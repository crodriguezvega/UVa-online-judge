module Problem_10556 =
  ...
  let isRotated polygon1 polygon2 =
    ...
    let angles = List.map2 angle polygon1 polygon2 |> List.tail
    let head = angles |> List.head
    angles |> List.forall (fun x -> abs x - head < 0.00001)

  let isScaled polygon1 polygon2 =
    let norm p = sqrt (pown p.y 2 + pown p.x 2) 
    let scale p1 p2 = norm p1 / norm p2
    let scales = List.map2 scale polygon1 polygon2 |> List.tail
    let head = scales |> List.head
    scales |> List.forall (fun x -> abs x - head < 0.00001)
